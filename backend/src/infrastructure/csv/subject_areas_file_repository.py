...
class SubjectAreasFileRepository(SubjectAreasRepository):
    ...
    def _load_areas(self) -> None:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                subject_subareas = row['SUBAREAS']

                # Procesar subáreas
                subareas = []
                if pd.notna(subject_subareas) and subject_subareas.strip():
                    # Dividir por ";" y limpiar espacios
                    subareas = [sub.strip() for sub in subject_subareas.split(';') if sub.strip()]
    ...
    def _belongs_to_subject_area(self, normalized_subject_subarea: str, area: SubjectArea) -> bool:
        ...
        for subarea in area.subareas:
            ...
            # Coincidencia exacta
            if normalized_subject_subarea == normalized_subarea:
                return True

            # Coincidencia parcial (la subárea está contenida en la definición)
            if normalized_subject_subarea in normalized_subarea:
                return True

            # Coincidencia parcial inversa (la definición está contenida en la subárea)
            if normalized_subarea in normalized_subject_subarea:
                return True
    ...
    def map_subarea_to_area(self, subarea: str) -> Optional[str]:
        ...
        Args:
            subarea: El nombre de la subárea a mapear (ejemplo: "Computer Science Applications")
        Returns:
