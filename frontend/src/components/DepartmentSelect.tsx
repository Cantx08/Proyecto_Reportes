...
const DepartmentSelect: React.FC<DepartmentSelectProps> = ({
  ...
  const { departments, isLoading, error: fetchError } = useDepartments();
  ...
  useEffect(() => {
    ...
    const filtered = departments.filter(dept => 
      ...
      dept.facultad.toLowerCase().includes(value.toLowerCase())
    );
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && !isLoading && !fetchError && filteredDepartments.length > 0 && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
          {filteredDepartments.map((department: Department) => (
            <div
              key={department.sigla}
              onClick={() => handleSelectDepartment(department)}
              className="px-3 py-2 cursor-pointer hover:bg-blue-50 hover:text-blue-700 border-b border-gray-100 last:border-b-0"
              title={`${department.nombre} - ${department.facultad}`}
            >
              <div className="font-medium">{department.nombre}</div>
              <div className="text-xs text-gray-500">{department.facultad}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {isOpen && !isLoading && !fetchError && value.trim() && filteredDepartments.length === 0 && (
