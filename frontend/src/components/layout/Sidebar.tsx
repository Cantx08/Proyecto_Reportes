...
const navigation = [
  ...
  { name: 'Autores', href: '/author_page', icon: Users },
  { name: 'Departamentos', href: '/departments_page', icon: Building2 },
  { name: 'Publicaciones', href: '/publications_page', icon: BookOpen },
  { name: 'Borradores', href: '/borradores', icon: FileEdit },
  { name: 'Informes Finales', href: '/reports_page', icon: ClipboardCheck },
];
            ...
            return (
              <li key={item.name}>
                ...
                >
                  ...
                  {isCollapsed && (
                    <span className="absolute left-16 bg-gray-900 text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20">
                      {item.name}
                    </span>
                  )}
                </Link>
